...
MeshCut3DUserObject::growFront()
{
  ...
  for (unsigned int i = 0; i < _active_boundary.size(); ++i)
  {
    ...
    std::cout << "_inactive_boundary_pos: ";
    for (std::vector<dof_id_type>::const_iterator i = _inactive_boundary_pos.begin(); i != _inactive_boundary_pos.end(); ++i)
        std::cout << *i << " ";
    std::cout << '\n';
  ...
  std::cout << "front: ";
  for (std::vector<dof_id_type>::const_iterator i = _front[0].begin(); i != _front[0].end(); ++i)
      std::cout << *i << " ";
  std::cout << '\n';
}
